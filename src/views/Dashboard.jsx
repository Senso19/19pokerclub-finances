...
export default function Dashboard({ ecritures, saison, categories }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-felt felt-texture rounded-3xl p-8 md:p-12 flex flex-col items-center shadow-card text-center">
        <span className="text-white/60 text-xs uppercase tracking-[0.2em] font-semibold mb-3">
          {saison?.nom || 'Saison en cours'}
        </span>
        <span className="text-white/70 text-sm mb-1">Trésorerie totale (banque + caisse)</span>
        <div className="font-mono text-4xl md:text-5xl font-bold tabular-nums text-white">
          {eur(soldeTotal)}
        </div>
        <div className="flex gap-8 mt-6 text-white/80 text-sm">
